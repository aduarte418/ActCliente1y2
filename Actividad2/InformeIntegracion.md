...
Tarda 0.06 s en pintar el título (en este caso, el mayor elemento visible que se renderiza). Eso sí, se destaca que en la consola salta el error de que no puede preparar las propiedades de un elemento que aún no existe (ya que no ha cambiado el elemento por acción del script), porque se ha ejecutado el script en el head, antes de que se hubiera renderizado el texto, situado en el body.
